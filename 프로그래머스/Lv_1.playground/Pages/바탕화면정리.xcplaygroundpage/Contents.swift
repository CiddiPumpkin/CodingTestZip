...
func solution(_ wallpaper:[String]) -> [Int] {
    let position = wallpaper.map({$0.map{String($0)}})
    let maxX = (position.first?.count ?? 0) - 1 > 0 ? (position.first!.count - 1) : 0
    ...
    luy = position.firstIndex(where: {$0.contains("#")}) ?? 51
    lux = position[0...maxY].map({$0.firstIndex(where: {$0 == "#"}) ?? 51}).min() ?? 51
    rdy = (position.lastIndex(where: {$0.contains("#")}) ?? -1) + 1
    rdx = (position[0...maxY].map({$0.lastIndex(where: {$0 == "#"}) ?? -1}).max() ?? -1) + 1
...
func solution(_ wallpaper:[String]) -> [Int] {
    ...
    for (i, paper) in wallpaper.enumerated() {
        for (j, file) in paper.enumerated() {
            if file == "#" {
                lux = min(lux, i)
                luy = min(luy, j)
                rdx = max(rdx, i)
